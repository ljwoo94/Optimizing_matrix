...
protected:
    ...
    //// C1 feature map
    int C1_channel = conv1_out_channel; //6
    int C1_size = input_size - (conv1_kernel_size - 1); //28
    //// S2 feature map
    int S2_channel = C1_channel; //6
    int S2_size = C1_size / 2; //14
    //// [Layer] conv2
    int conv2_in_channel = conv1_out_channel; //6
    int conv2_out_channel = 16;
    int conv2_kernel_size = 5;
    //// C3 feature map
    int C3_channel = conv2_out_channel; //16
    int C3_size = S2_size - (conv2_kernel_size - 1); //10
    //// S4 feature map
    int S4_channel = C3_channel; //16
    int S4_size = C3_size / 2; //5
    //// [Layer] fc1
    int fc1_in_channel = S4_channel * S4_size * S4_size; //400
